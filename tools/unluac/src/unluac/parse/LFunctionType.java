...
abstract public class LFunctionType extends BObjectType<LFunction> {
  ...
  protected void parse_debug(ByteBuffer buffer, BHeader header, LFunctionParseState s) {
    try {
      ...
      // Some real-world chunks have truncated/malformed debug sections.
      // Treat as a stripped chunk and continue, so the function can still be disassembled/decompiled.
      if(header.debug) {
        System.out.println("-- truncated debug section; treating as stripped");
      }
      s.lines = new BList<>(new BInteger(0), Collections.emptyList());
      s.locals = new BList<>(new BInteger(0), Collections.emptyList());
    }
  }
