...
add_button(main_tab.hdr, "开启bloom", 0)
add_button(main_tab.hdr, "开启star", 100)
add_button(main_tab.hdr, "返回大厅", 200)
btn_ui[1].Visible = false
add_slider(main_tab.hdr, 0, 1, 1, "       scene强度")
add_slider(main_tab.hdr, 0.5, 1, 0.7, "       渐变速度")
add_pad(main_tab.hdr, 30, "bloom setting")
add_slider(main_tab.hdr, 0, 1, 1, "       bloom强度")
add_slider(main_tab.hdr, 0, 1, 0.5, "       中灰")
add_slider(main_tab.hdr, 0, 5, 0.8, "       亮度过滤")
add_slider(main_tab.hdr, 0, 5, 0.5, "       亮度偏移")
add_pad(main_tab.hdr, 30, "star setting")
add_slider(main_tab.hdr, 0, 1, 1, "       star强度")
add_slider(main_tab.hdr, 0.7, 1, 0.8, "       star衰减")
add_slider(main_tab.hdr, 0, 3.14, 0, "   star角度")
add_slider(main_tab.hdr, 0.2, 4, 3, "       star长度")
...
add_slider(main_tab.shadow, -1, 1, 1, "阴影方向 x")
add_slider(main_tab.shadow, -1, 1, 1, "阴影方向 y")
add_slider(main_tab.shadow, -1, 1, 1, "阴影方向 z")
add_button(main_tab.CA, "开启颜色校正", 100)
add_slider(main_tab.CA, 0, 2, 1, "亮度")
add_slider(main_tab.CA, 0, 3, 1, "对比度")
...
btn_func_list[2] = function()
  ...
  b_star = not b_star
  if b_star then
    btn_ui[2].Text = "开启star"
  else
    btn_ui[2].Text = "关闭star"
  end
end
...
btn_func_list[4] = function()
  ...
  b_CA = not b_CA
  if b_CA then
    btn_ui[4].Text = "开始颜色校正"
  else
    btn_ui[4].Text = "关闭颜色校正"
  end
end
