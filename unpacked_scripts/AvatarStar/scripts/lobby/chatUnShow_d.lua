...
local UpdateUnShowChatList, Initialize = function()
  ...
  while true do
    ...
    list:AddSubItem(item, chatItem.Name)
    item:SetTextColor(1, ARGB(255, 255, 255, 255))
    item:SetHighLightTextColor(1, ARGB(255, 255, 255, 0))
    list:AddSubItem(item, chatItem.Type)
    list:AddSubItem(item, chatItem.ChannelID)
    list:AddSubItem(item, chatItem.ID)
    list:AddSubItem(item, chatItem.OwerID)
    if selected_item and tonumber(selected_item:GetText(2)) == chatItem.Type and tonumber(selected_item:GetText(3)) == chatItem.ChannelID and tonumber(selected_item:GetText(4)) == chatItem.ID then
