module("GuildTeamEnlarge", package.seeall)
...
local ui = Gui.Create()({
  ...
  Gui.Control("main")({
    ...
    Gui.Control({
      Size = Vector2(390, 40),
      ComFuc.ComLabel(nil, "  " .. GetUTF8Text("button_common_war_03"), Vector2(382, 21), Vector2(4, 4), 0, 16, colw),
    ...
    Gui.Control({
      ...
      ComFuc.ComLabel("label_1", GetUTF8Text("UI_common_enlarge_01"), Vector2(266, 36), Vector2(18, 13), 0, 16, colt, "kAlignLeftMiddle"),
      ComFuc.ComLabel("label_2", GetUTF8Text("UI_common_enlarge_02"), Vector2(266, 36), Vector2(18, 50), 0, 16, colt, "kAlignLeftMiddle"),
      ComFuc.ComLabel("label_3", GetUTF8Text("UI_common_enlarge_03"), Vector2(266, 36), Vector2(18, 86), 0, 16, colt, "kAlignLeftMiddle"),
    ...
    Gui.Control("coin_content")({
      ...
      BackgroundColor = colw,
      Skin = SkinF.battle_005,
      ComFuc.ComLabel(nil, GetUTF8Text("UI_common_enlarge_04"), Vector2(330, 24), Vector2(18, 12), 0, 16, colt),
      ComCoinItem(1),
      ComCoinItem(2),
      ComCoinItem(3),
      ComCoinItem(4)
    }),
    ComFuc.ComButton("btn_enlarge", GetUTF8Text("button_common_war_03"), Vector2(140, 53), Vector2(125, 325), 16, false, true, SkinF.select_character_038)
  })
})
...
function ui.btn_enlarge.EventClick()
  ...
  rpc.safecall("guild_team_expansion", {
...
function Show(info)
  rpc.safecall("guild_team_expansion_detail", {
    guildId = info[3]
  }, function(data)
    enlargeInfo = {}
    enlargeInfo.id = info[3]
    local dataList = data.list
    ui.text_1.Text = info[2]
    if dataList and dataList[1] and dataList[1].teamNum then
      ui.text_2.Text = dataList[1].teamNum
      ui.label_2.Text = GetUTF8Text("UI_common_enlarge_02")
    else
      ui.text_2.Text = ""
      ui.label_2.Text = GetUTF8Text("msgbox_common_backpack_02")
    end
    ui.text_3.Text = data.sysMaxGuildTeamNum
    if dataList then
      for i = 1, 4 do
        local v = dataList[i]
