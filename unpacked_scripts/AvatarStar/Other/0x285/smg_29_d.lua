weapon:SetMesh("rv", "smg_29/rv1_lod0.mesh")
weapon:SetMesh("mz", "smg_29/mz1_lod0.mesh")
weapon.skeleton = "/skeleton/smg_08.skel"
weapon.animation_set = "smg_08"
weapon.icon = texture("/ui/skinF/lobby/smg_29.tga")
weapon.kill_icon = texture("/ui/weapon/mk18_01.dds")
weapon.ammo_icon = texture("/ui/ingameF/skin_ingame_icon_ammoBG_row.tga")
weapon.cross_hair_icon = texture("/ui/weapon/smg.dds")
weapon.is_sight = true
weapon.bullet_particle_first = "mk18_bullets_out_1st"
weapon.bullet_particle_third = "bullets_out_3rd_christmas"
weapon.name = "smg_01"
weapon.sound_name = "smg"
weapon.reload_sound = "reload2"
weapon.fire_particle = "smg_01_mzflash"
weapon.attach_particle = "q_smg"
