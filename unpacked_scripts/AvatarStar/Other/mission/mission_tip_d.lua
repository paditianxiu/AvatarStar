...
function UpdateReward(data)
  ...
  np_get_reward:PushBlockToLastLine(_T("UI_store_Congratulations") .. "   ", ARGB(255, 255, 255, 0), false, "", ARGB(0, 0, 0, 0))
  np_get_reward:PushBlockToLastLine(_T("UI_store_Obtain") .. "   ", ARGB(255, 255, 255, 255), false, "", ARGB(0, 0, 0, 0))
  for i, v in ipairs(data.reward) do
    ...
    if v.type == 7 then
      ...
      np_get_reward:PushBlockToLastLine(" " .. GetCurrencyText(c), GetGradeColor(1), false, "", ARGB(0, 0, 0, 0))
      reward_ui[i].Icon = currency_icon[c]
      reward_ui[i].GradeImage = GetGradeImage(1)
      reward_ui[i].Count = v.unit
    else
      np_get_reward:PushBlockToLastLine(" " .. _T(v.displayName), GetGradeColor(v.grade), false, "", ARGB(0, 0, 0, 0))
      reward_ui[i].Icon = Tip.GetIcon(v.resource)
      reward_ui[i].GradeImage = GetGradeImage(v.grade)
