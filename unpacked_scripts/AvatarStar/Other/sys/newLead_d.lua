...
ui = Gui.Create()({
  ...
  ComFuc.ComButton("btn_skip", GetUTF8Text("UI_common_Task_guide_01"), Vector2(300, 40), Vector2(500, 850), 16, false),
  Gui.Control("skill_f")({
...
function SetComNewLeadLock(a, b, t, d, isSizeChanged, arrow_offset)
  leadA = a
  leadB = b
  leadT = t
  leadD = d
  ...
  if d == 0 then
    ...
    if arrow_offset == nil then
      ...
      ui.Arrow_f.FirstLc = Vector2(a.x + arrow_offset - 22.8 + ComFuc.locationChanged, a.y - 58.400000000000006 - 8)
      ui.AutoText_f.MiddCenter = Vector2(a.x + arrow_offset + ComFuc.locationChanged, a.y - 56)
    end
  elseif d == 1 then
    ui.Arrow_f.Size = Vector2(45.6, 58.400000000000006)
    ...
    if arrow_offset == nil then
      ...
      ui.Arrow_f.FirstLc = Vector2(a.x + arrow_offset - 22.8 + ComFuc.locationChanged, a.y + b.y - 8)
      ui.AutoText_f.MiddCenter = Vector2(a.x + arrow_offset + ComFuc.locationChanged, a.y + b.y + 56)
    end
  elseif d == 2 then
    ...
    if arrow_offset == nil then
      ...
      ui.Arrow_f.FirstLc = Vector2(a.x + b.x + ComFuc.locationChanged, a.y + arrow_offset - 22.8)
      ui.AutoText_f.MiddCenter = Vector2(a.x + b.x + 70 + ComFuc.locationChanged, a.y + arrow_offset)
    end
  elseif d == 3 then
    ui.Arrow_f.Size = Vector2(58.400000000000006, 45.6)
    ...
    if arrow_offset == nil then
      ...
      ui.Arrow_f.FirstLc = Vector2(a.x - 58.400000000000006 + ComFuc.locationChanged, a.y + arrow_offset - 22.8)
      ui.AutoText_f.MiddCenter = Vector2(a.x - 50 + ComFuc.locationChanged, a.y + arrow_offset)
    end
  elseif d == 4 then
...
function ShowNewLeadNoLock(a, b, t, d, isSizeChanged)
  ...
  if ChatBar then
    ChatBar.EnforceReshow()
  end
  if ChatUnShow then
    ChatUnShow.EnforceReshow()
  end
  ui.btn_skip.Parent = nil
  ui.btn_skip.Visible = false
end

local ShowNewLeadEmptyHasParent, LockGlobal = function(a, b, p)
  ui.region_f.Size = b + Vector2(28, 28)
...
function LockGlobal(a, b, p)
  ...
  ui.btn_skip.Parent = gui
  ui.btn_skip.Visible = true
end

function ShowNewLeadEmptyHasLockHasParent(a, b, p)
  print("--debug-- lock, p", p)
  ui.region_f.Size = b + Vector2(28, 28)
  ui.region_f.Location = a - Vector2(14, 14)
  ui.region_f.Parent = p
  leadVisible = true
  a = p:ClientToScreen(a)
  LockGlobal(a, b, p)
end
...
function ShowNewLeadHasLock(a, b, t, d, isSizeChanged, arrow_offset)
  leadFuc = ShowNewLeadHasLock
  ShowEmptyHasLock(a, b, t, d, isSizeChanged, false)
  SetComNewLeadLock(a, b, t, d, isSizeChanged, arrow_offset)
end
...
function ShowHoldUi()
  if ComFuc.hookNum >= 1 then
    local text = string.format(GetUTF8Text("msgbox_inGame_debuff_parasite"), ComFuc.hookNum)
    MessageBox.Show(text, GetUTF8Text("button_common_OK"), function()
      if ComFuc.hookNum >= 3 then
        Thread.Quit()
      end
      ComFuc.hookNum = 0
    end)
...
function ui.btn_skip.EventClick(sender, e)
  MessageBox.ShowWithConfirmCancel(GetUTF8Text("msgbox_common_Task_guide_18"), SkipForceLead)
end
