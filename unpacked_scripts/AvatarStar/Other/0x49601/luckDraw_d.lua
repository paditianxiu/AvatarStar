...
ui = Gui.Create()({
  ...
  Gui.Control("mainCtrl_2")({
    ...
    ComFuc.ComButton("close_2", nil, Vector2(24, 24), Vector2(368, 5), 0, false, false, SkinF.lookInfo_002),
...
function DealGoodsGet(data)
  ...
  for i = 1, #data.list do
    if not data.list[i].isAppendage then
      cat = data.list[i].category
      break
    end
  end
  while true do
    ...
    if needstep < 50 or #lcmap[cat] < 2 then
      break
    end
  end
...
ui.close_2.EventClick = BeginDrawnNum
